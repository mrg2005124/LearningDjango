...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if parent_email in [ user_email]:
            parent_email = False

        if author_email:
            email = EmailMessage(
                'دیدگاه جدید',
                f"دیدگاه جدید برای مقاله (|{article}|)ثبت شد . {current_site} {reverse('blog:detail',kwargs={'slug' : article.slug})}",
                to = [author_email]
            )
            email.send()
        ...
        if user_email:
            email = EmailMessage(
                'دیدگاه شما ثبت شد',
                'دیدگاه شما ثبت شد و بزودی به آن پاسخ داده خواهد شد',
                to = [user_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                'پاسخی برای نظر شما ثبت شد.',
                f'برای نظری که شما در رابطه با مقاله (|{article}|)داده بودید  {self.comment.user.first_name} {self.comment.user.last_name}  پاسخی ثبت کرد. ',
                to = [parent_email]
            )
            email.send()
